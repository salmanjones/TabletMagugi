...
export default React.memo(({reserveInfoArray, checkStylistEvent, reserveFlag}) => {
    ...
    const Item = React.memo(({index, title, validCount, invalidCount, staffId}) => {
        ...
        if (checkStaffId && checkStaffId.length > 0) {
            return (
                <TouchableOpacity style={ReserveBoardStyles.reserveStylistItemBox} onPress={() => {
                    checkItemEvent(index, staffId)
                }}>
    ...
    if (reserveInfoArray.length > 0) {
        return (
            <FlatList
                ...
                renderItem={
                    ({item, index}) => <Item staffId={item.staffId} index={index} title={item.staffName} validCount={item.staffNowReseverCount}
                                             invalidCount={item.staffPassReseverCount}/>
                }
